...
class WC_COG_Admin_Report_Profit_by_Product extends WC_COG_Admin_Report {
	...
	protected function set_product_ids() {
		...
		$this->product_ids = isset( $_GET['product_ids'] ) ? array_filter( array_map( 'absint', (array) $_GET['product_ids'] ) ) : [];
	}
	...
	public function get_chart_widgets() {
		...
		$widgets = [
			...
			[
				'title'    => '',
				'callback' => [ $this, 'output_profitable_sellers_widget' ],
			],
		];
		...
		<div class="section">
			<form method="GET">
				<div>
					...
					<input type="hidden" name="end_date" value="<?php if ( ! empty( $_GET['end_date'] ) ) {
						echo esc_attr( $_GET['end_date'] );
					} ?>" />
					<input type="hidden" name="page" value="<?php if ( ! empty( $_GET['page'] ) ) {
						echo esc_attr( $_GET['page'] );
					} ?>" />
					<input type="hidden" name="tab" value="<?php if ( ! empty( $_GET['tab'] ) ) {
						echo esc_attr( $_GET['tab'] );
					} ?>" />
					<input type="hidden" name="report" value="<?php if ( ! empty( $_GET['report'] ) ) {
	...
	public function output_profitable_sellers_widget() {
		...
		<div class="section">
			<table cellspacing="0">
				<?php
				$sellers = $this->get_order_report_data( [
					'data'         => [
						'_line_total'             => [
					...
					var series = [
						{
							...
							bars       : {
								fillColor : '<?php echo $this->chart_colors['total_items']; ?>',
								fill      : true,
								show      : true,
								lineWidth : 0,
	 ...
	 * @return stdClass
	 */
	protected function get_report_data() : stdClass {
		...
		// sales
		$this->report_data->sales = $this->get_order_report_data( [
			'data'         => [
				'_line_total' => [
					'type'            => 'order_item_meta',
					'order_item_type' => 'line_item',
			...
			'where_meta'   => [
				...
				[
					'type'       => 'order_item_meta',
					'meta_key'   => [ '_product_id', '_variation_id' ],
		...
		// COGS
		$this->report_data->cogs = $this->get_order_report_data( [
			'data'         => [
				'_wc_cog_item_total_cost' => [
					'type'            => 'order_item_meta',
					'order_item_type' => 'line_item',
		...
		// profit
		$this->report_data->profits = $this->get_order_report_data( [
			'data'         => [
				'_line_total'             => [
					'type'            => 'order_item_meta',
					'order_item_type' => 'line_item',
		...
		// item counts
		$this->report_data->item_counts = $this->get_order_report_data( [
			'data'         => [
				'_qty'        => [
					'type'            => 'order_item_meta',
					'order_item_type' => 'line_item',
