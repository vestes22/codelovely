...
class WC_COG_Admin_Report_Total_Valuation {
	...
	public function output_report() {
		...
		$valuation = [ 'at_cost' => 0, 'at_retail' => 0 ];

		// loader image
		$loader_image = wc_cog()->get_framework_assets_url() . '/images/ajax-loader.gif';

		wp_enqueue_style( 'wc-cog-valuation', wc_cog()->get_plugin_url() . '/assets/css/admin/wc-cog-valuation.min.css', [], WC_COG::VERSION );
		?>
		...
		if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) :
			...
			<?php
			wp_enqueue_script( 'wc-cog-total-valuation', wc_cog()->get_plugin_url() . '/assets/js/admin/wc-cog-total-valuation.min.js', [ 'jquery' ], WC_COG::VERSION );
			wp_localize_script( 'wc-cog-total-valuation', 'wc_cog_total_valuation', [
