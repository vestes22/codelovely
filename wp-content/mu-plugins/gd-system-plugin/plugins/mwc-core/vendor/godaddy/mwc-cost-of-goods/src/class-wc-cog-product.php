...
class WC_COG_Product {
	 ...
	 */
	public static function get_variable_product_min_max_costs( $product ) : array {
		...
		return [ $min_variation_cost, $max_variation_cost ];
	}


	/**
	 * Returns the average cost associated with the child variations of a given product
	 *
	 * @since 3.0.0
	 *
	 * @param WC_Product_Variable|int $product the variable product
	 * @return float|string product average cost if variations' costs are configured, the empty string otherwise
	 */
	public static function get_variable_product_average_costs( $product ) {
		...
		return count( $costs ) > 0 ? array_sum( $costs ) / count( $costs ) : '';
	}
	 ...
	 */
	public static function get_cost_html( $product ) : string {
