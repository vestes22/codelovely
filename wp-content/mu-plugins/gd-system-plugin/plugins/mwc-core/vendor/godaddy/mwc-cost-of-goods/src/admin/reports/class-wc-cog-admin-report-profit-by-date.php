...
class WC_COG_Admin_Report_Profit_by_Date extends WC_COG_Admin_Report {
	...
	public function get_export_button() {
		...
		$this->output_export_button( [ 'exclude_series' => 4 ] );
	}
	...
	public function get_main_chart() {
		...
		$net_order_amounts = $profit_amounts = [];
		...
		$chart_data = [
			'net_order_amounts' => array_map( [ $this, 'format_decimal' ], array_values( $net_order_amounts ) ),
			'cogs_amounts'      => array_map( [ $this, 'format_decimal' ], array_values( $cogs_amounts ) ),
			'profit_amounts'    => array_map( [ $this, 'format_decimal' ], array_values( $profit_amounts ) ),
					...
					var series = [
						{
							...
							bars       : {
								fillColor : '<?php echo $this->chart_colors['total_orders']; ?>',
								fill      : true,
								show      : true,
								lineWidth : 0,
	...
	protected function get_report_data() : stdClass {
		...
		$this->report_data->orders = (array) $this->get_order_report_data( [
			...
			'filter_range'        => true,
			'order_types'         => array_merge( [ 'shop_order_refund' ], wc_get_order_types( 'sales-reports' ) ),
		...
		$this->report_data->fees = (array) $this->get_order_report_data( [
			'data'                => [
				'_line_total' => [
		...
		$this->report_data->cogs = (array) $this->get_order_report_data( [
			'data'                => [
				'_wc_cog_order_total_cost' => [
					'type'     => 'meta',
					'function' => 'SUM',
					'name'     => 'total_cogs',
				],
		...
		$this->report_data->orders_count = (array) $this->get_order_report_data( [
			'data'         => [
				'ID'        => [
					'type'     => 'post_data',
					'function' => 'COUNT',
