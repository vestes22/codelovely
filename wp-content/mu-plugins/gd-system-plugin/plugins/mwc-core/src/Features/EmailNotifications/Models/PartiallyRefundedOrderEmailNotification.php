...
/**
 * Model for partially refunded order email notification.
 */
class PartiallyRefundedOrderEmailNotification extends EmailNotification implements OrderEmailNotificationContract
{
    use IsOrderEmailNotificationTrait;
    use IsCustomerEmailNotificationTrait;

    /** @var string */
    protected $id = 'customer_partially_refunded_order';
    ...
    public function __construct()
    {
        $this->setName($this->getId())
            ->setLabel(__('Partially refunded order', 'mwc-core'))
            ->setDescription(__('Sent to customers when their order is partially refunded.', 'mwc-core'));
    }
    ...
    protected function getMainContent() : string
    {
        ...
        <?php printf(
            ...
            esc_html__('Your order on %s has been partially refunded. There are more details below for your reference:', 'mwc-core'),
    ...
    protected function getInitialSettings() : array
    {
        return [
            ...
            $this->getSubjectSettingObject()
                ->setDefault(__('Your {{site_title}} order #{{order_number}} has been partially refunded', 'mwc-core')),
